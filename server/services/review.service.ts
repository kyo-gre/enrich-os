...
export async function approveCreator(creatorId: string): Promise<void> {
  await setReviewStatus(creatorId, "approved");
  await addProcessingLog({ creatorId, step: "review_approved", status: "success" });
}

export async function ignoreCreator(creatorId: string): Promise<void> {
  await setReviewStatus(creatorId, "ignored");
  await addProcessingLog({ creatorId, step: "review_ignored", status: "success" });
}
...
async function createFallbackIdentityCache(creator: CreatorRow): Promise<string> {
  ...
  const row = await createIdentityCache({
...
export async function applyCreatorOverride(
  ...
  const identityCacheId =
    creator.identity_cache_id ?? (await createFallbackIdentityCache(creator));

  await applyManualOverride(identityCacheId, field, oldValue, newValue, reason);

  await applyResolvedIdentity(creatorId, {
 ...
 */
export async function mergeDuplicateCreators(
  sourceId: string,
  targetId: string,
): Promise<void> {
  ...
  await setReviewStatus(sourceId, "approved");
 ...
 */
export async function unmergeCreator(creatorId: string): Promise<CreatorRow> {
  const creator = await getCreator(creatorId);
  if (!creator) throw new Error(`Creator not found: ${creatorId}`);
  ...
  const logs = await listProcessingLogsForCreator(creatorId);
  ...
  await clearDuplicateOf(creatorId);
  await applyResolvedIdentity(creatorId, {
  ...
  await setReviewStatus(creatorId, preMergeSnapshot.reviewStatus);
