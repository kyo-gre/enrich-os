...
export async function getCreator(id: string): Promise<CreatorRow | undefined> {
  const result = await libsqlClient.execute({
    sql: "SELECT * FROM creators WHERE id = ?",
    args: [id],
  });
  return result.rows[0] as unknown as CreatorRow | undefined;
}

export async function listCreatorsByImport(
  importId: string,
): Promise<CreatorRow[]> {
  const result = await libsqlClient.execute({
    sql: "SELECT * FROM creators WHERE import_id = ? ORDER BY row_index ASC",
    args: [importId],
  });
  return result.rows as unknown as CreatorRow[];
}
...
export async function getCreatorStats(importId: string): Promise<CreatorStats> {
  const result = await libsqlClient.execute({
    sql: `SELECT
        COUNT(*) AS processed,
...
export async function applyResolvedIdentity(
  id: string,
...
export async function setRawMappedFields(
  id: string,
  fields: RawMappedFields,
): Promise<void> {
  await libsqlClient.execute({
    sql: `UPDATE creators SET
...
export async function setReviewStatus(
  ...
  await libsqlClient.execute({
    sql: "UPDATE creators SET review_status = ?, updated_at = ? WHERE id = ?",
...
export async function markDuplicateOf(
  ...
  await libsqlClient.execute({
    sql: "UPDATE creators SET duplicate_of_creator_id = ?, updated_at = ? WHERE id = ?",
    args: [targetCreatorId, Date.now(), id],
  });
}

export async function clearDuplicateOf(id: string): Promise<void> {
  await libsqlClient.execute({
    sql: "UPDATE creators SET duplicate_of_creator_id = NULL, updated_at = ? WHERE id = ?",
    args: [Date.now(), id],
  });
}
